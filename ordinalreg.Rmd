...
```{r}
predictedstars <- predict(olog, dftest, type="p")  # predict the probabilites
head(predictedstars)
```

```{r}
predcheat <- rep(NA, nrow(predictedstars))
...
predcheat <- predictcheat(predictedstars)
predcheat
```
...
```{r}
conf_olog_cheat <- table(dftest$starsfactor, predcheat)
conf_olog_cheat
