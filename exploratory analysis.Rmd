...
```{r}
# FULL DATA SET (find a way to make this nicer)
yelp_train$prunedtext <- lapply(yelp_train$text, FUN = dropStopwords)
...
# Loops over each sentiment variable created in the above chunk
# and generates sentimentratio = sentiment/nwords in new column
for(sent in sentiments){
  ratio <- paste0(sent, "ratio")
  yelp_train[ratio] <- unlist(yelp_train[sent]) / unlist(yelp_train["nwords"])
}
...
ggplot(yelp_train, aes(starsfactor, joy)) + 
  geom_bar(stat = "identity")
...
antnum <- as.numeric(yelp_train$anticipation)
is.na(yelp_train$stars)
```
